import type { ExecutionStep, ExecutionTrace, TraceTensor } from "./modelTrace.ts";
...
export function bindExecutionWorkspace(kind: RealModelKind, typesetMath: TypesetMath): void {
  ...
  const execute = async (inference: boolean) => {
    ...
    try {
      ...
      if (!current()) return;
      output.innerHTML = `<small>${escapeHtml(trace.outputLabel)}</small><strong>${escapeHtml(trace.output)}</strong>`;
      runtime.textContent = `${trace.backend} · ${trace.elapsed.toFixed(1)} ms including trace capture`;
      disposeTrace = bindTrace(result, trace, typesetMath);
...
function bindTrace(root: HTMLElement, trace: ExecutionTrace, typesetMath: TypesetMath): () => void {
  const sliceSize = 32;
  let selected = 0;
  let tensorIndex = 0;
  let tensorName = "";
  ...
  root.innerHTML = `<section class="dl-execution" aria-label="Recorded forward pass">
    <div class="dl-transport"><button type="button" data-restart aria-label="Restart walkthrough">↺</button><button type="button" data-prev>← Previous</button><button type="button" data-play>Play</button><button type="button" data-next>Next →</button><label class="dl-position"><span></span><input id="execution-step" aria-label="Execution step" type="range" min="0" max="${trace.steps.length - 1}" value="0"></label></div>
    ...
    <div class="dl-layout"><nav class="dl-step-list" aria-label="Forward pass steps">${trace.steps.map((step, index) => `<button type="button" data-step="${index}"><small>${String(index + 1).padStart(2, "0")}</small><span>${escapeHtml(step.name)}</span></button>`).join("")}</nav><section class="dl-detail" aria-label="Current operation"></section></div>
  ...
  const drawCandidate = () => {
    ...
    container.innerHTML = `<strong>${escapeHtml(visibleToken(candidate.token))}</strong><span>p = <code>${candidate.probability}</code>${candidate.logit === undefined ? "" : ` · logit = <code>${candidate.logit}</code>`}</span>`;
  ...
  const attention = (step: ExecutionStep) => {
    ...
    return `<section class="dl-attention"><h3>Attention <small>Output positions ↓ · source positions →</small></h3><div class="dl-attention-scroll"><table><thead><tr><th scope="col">Output</th>${map.source.map((token, column) => `<th scope="col"><button type="button" data-source="${column}" aria-pressed="${column === sourceIndex}"><small>${column}</small>${escapeHtml(visibleToken(token))}</button></th>`).join("")}</tr></thead><tbody>${map.weights.map((row, rowIndex) => `<tr class="${rowIndex === map.row ? "is-active" : ""}"><th scope="row"><button type="button" data-row="${rowIndex}" aria-pressed="${rowIndex === map.row}"><small>${rowIndex}</small>${escapeHtml(visibleToken(map.target[rowIndex] ?? ""))}</button></th>${row.map((weight, column) => `<td class="${column === sourceIndex ? "is-source" : ""}"><button type="button" data-cell="${rowIndex},${column}" tabindex="${rowIndex === map.row && column === sourceIndex ? 0 : -1}" aria-pressed="${rowIndex === map.row && column === sourceIndex}" aria-label="Output ${rowIndex} ${escapeHtml(visibleToken(map.target[rowIndex] ?? ""))}, source ${column} ${escapeHtml(visibleToken(map.source[column]))}, weight ${weight}" style="background:${cellColor(weight, 1)};color:${weight > .55 ? "white" : "var(--ink)"}">${weight.toFixed(2)}</button></td>`).join("")}</tr>`).join("")}</tbody></table></div><div class="dl-attention-detail dl-value-detail" aria-live="polite"></div><p class="dl-note">Colour uses the original weight, from 0 to 1. Cells are rounded; select a cell for its stored value.</p></section>`;
  };
  ...
  const drawRecurrence = () => {
    ...
    container.innerHTML = `<header><strong>Memory update</strong><span>Unit ${unit} · rounded values</span></header><div class="dl-memory-lane"><span class="dl-lane-label">Cell memory</span>${term("Previous cell memory", "cₜ₋₁")}<b>×</b>${term("Forget gate", "fₜ")}<b>+</b>${term("Input gate", "iₜ")}<b>×</b>${term("Candidate memory", "c̃ₜ")}<b>→</b>${term("Cell memory", "cₜ")}</div><div class="dl-hidden-lane"><span class="dl-lane-label">Exposed state</span>${term("Output gate", "oₜ")}<b>× tanh(</b>${term("Cell memory", "cₜ")}<b>) →</b>${term("Hidden state", "hₜ")}<span class="dl-memory-delta">Δ c[${unit}] = <code>${number(value("Cell memory") - value("Previous cell memory"))}</code></span></div>`;
  };
  ...
  root.oninput = event => {
    if (event.target === slider) { stop(); move(Number(slider.value)); }
  };
  root.onchange = event => {
  ...
  root.onkeydown = event => {
    ...
    } else if (!event.altKey && !event.ctrlKey && (event.key === "ArrowLeft" || event.key === "ArrowRight")) {
